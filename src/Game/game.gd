...
func _setup_speedrun() -> void:
	...
	if Globals.is_new_game:
		...
		var level_time_label: Label = $SpeedrunUI/Control/VBoxContainer.get_node(
...
func _load_level() -> void:
	...
	var level_scene: PackedScene = ResourceLoader.load(
		"res://src/Game/Level/Levels/Level" + str(Globals.level) + ".tscn"
	)
...
func _process(_delta: float) -> void:
	if Globals.is_speedrun_mode and is_counting_time:
		...
		Globals.level_speedrun_times[6] = (
			Globals.level_speedrun_times[1]
			+ Globals.level_speedrun_times[2]
			+ Globals.level_speedrun_times[3]
			+ Globals.level_speedrun_times[4]
			+ Globals.level_speedrun_times[5]
		)
